...
void waterQualityWindow::createStatusBar()
{
    ...
    // Compliance data uploaded label
    ...
    complianceCheckBox->setEnabled(false); // Prevent manual toggling
    complianceCheckBox->setChecked(false); // Default unchecked
...
void waterQualityWindow::updateProgressBar(QTimer* timer, const QString& successMessage)
{
    ...
    connect(timer, &QTimer::timeout, [=, this]() mutable {
        ...
        if (progressValue >= 99) {
            ...
            statusBar()->showMessage(successMessage, 5000); // Show message for 5 seconds
        }
    });

    timer->start(400); // Update progress bar every 400 milliseconds
}
...
void waterQualityWindow::uploadRealData()
{
    ...
    try {
        ...
        updateProgressBar(timer, "Real data loaded successfully!");
