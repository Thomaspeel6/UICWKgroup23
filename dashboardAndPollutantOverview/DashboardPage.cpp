...
DashboardPage::DashboardPage(): QMainWindow(), statsDialog(nullptr), pollutantPage(nullptr)
{
    setupDashboard();
    createButtons();
    createToolBar();
    ...
    setMinimumSize(MIN_WIDTH, MIN_WIDTH * 0.75);
    setWindowTitle("Water Quality Tool");
}


void DashboardPage::setupMainDashboard()
{
    ...
    QString style = "margin-top: 0%; font: bold 24px; text-align: center;";
    ...
    // connect
    connect(pollutantBtn, &QPushButton::clicked, this, &DashboardPage::showPollutantPage);
}

void DashboardPage::showPollutantPage()
{
    ...
    setCentralWidget(pollutantPage);
    
    if (model.hasData()) {
        pollutantPage->updateData();
    }
}
